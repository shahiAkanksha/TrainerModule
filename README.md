...
This project as implementation of a modern Android application using Jetpack Compose, Room, DataStore Preferences and Navigation Compose. The app shows a listscreen with tiles of tasks where thhe tiles contain title, description, and completion status that stays persistent with app restarts. Click on any tile navigates to a detail screen where users can toggle the task status. The state for completion status persists even after app restarts through RoomDB and DataStore. The solution follows a clean separation of concerns, scalability, and modern best practices.
...
**3. Architecture Used – MVVM With Repository**
The application follows the MVVM pattern with a Repository layer, ensuring a structured and maintainable codebase.
...
acts as a mediator between UI and Repository.
Also,Survives configuration changes.

**c)Repository (Data Handling Layer)**
Abstracts data is accessible from multiple data sources:
RoomDB for persistent valu of completion status.
DataStore for user preference for filtering the status (Completed/Pending).
This creates a single source of data.
Tis allows future scalability with using network calls may be through different network sources.

**4. Why This Architecture Was Chosen**
a) MVVM with repository fits perfectly suitable with Jetpack Component and Jetpac Compose. Also this is a Google recommended approach for scalability in Android apps.
b) There happens to be a separation of responsibilities and tasks to be done such as, UI logic stays in ViewModel, Business logic stays in repository, Data persistence stays in Room/DataStore.This ensures that each part is testable. 
...
e) Using Room + DataStore ensures data persists across app sessions, even after killing the app withhelp of viewmodel
...
**5. Features Implemente**
 ...
 --Room entity + DAO + repository mapping
